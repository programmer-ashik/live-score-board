...
const SYNC_CHANNEL = "cricket_sync_channel";
...
// Helper functions
function getBattingSquad(state) {
  return state.battingTeam === "teamA"
    ? state.teams.teamA.squad
    : state.teams.teamB.squad;
}
function getBowlingSquad(state) {
  return state.battingTeam === "teamA"
    ? state.teams.teamB.squad
    : state.teams.teamA.squad;
}
...
function getOpponentTeam(state) {
...
function getCurrentBowler(state) {
...
function formatBowlerOvers(totalBalls) {
...
function calculateCRR(runs, overs, balls) {
  ...
  return totalOvers === 0 ? "0.00" : (runs / totalOvers).toFixed(2);
}
function calculateRRR(target, runs, ballsLeft) {
  return ballsLeft === 0
    ? "0.00"
    : (((target - runs) / ballsLeft) * 6).toFixed(2);
}
...
function ControllerDashboard() {
  ...
  useEffect(() => {
    ...
    channel.onmessage = (event) => {
      ...
      } else if (event.data.type === "BOUNDARY") {
        setBoundaryAlert(event.data);
        setTimeout(() => setBoundaryAlert(null), 3000);
      } else if (event.data.type === "ALERT") {
        setWicketAlert(event.data);
        setTimeout(() => setWicketAlert(null), 3500);
      }
    };
  ...
  // Save state to localStorage and broadcast
  ...
  const registerWicket = useCallback(
    (playerId, dismissalType, incomingId) => {
      pushHistory();
      updateState((s) => {
        const batSquad = getBattingSquad(s);
        const bowlSquad = getBowlingSquad(s);
        ...
        if (dismissed) {
          ...
          if (newBatsman) {
            ...
            if (playerId === s.activePlayers.bat1Idx)
              s.activePlayers.bat1Idx = incomingId;
          ...
          channel.postMessage({
            type: "ALERT",
            title: "WICKET!",
            subtitle: `${dismissed.name} ${dismissed.runs}(${dismissed.balls})`,
          });
          channel.close();
        }
      });
  ...
  const setBattingTeam = useCallback(
    (team) =>
      updateState((s) => {
        s.battingTeam = team;
      }),
  ...
  const setTarget = useCallback(
    (target) =>
      updateState((s) => {
        s.target = target ? parseInt(target) : null;
      }),
    [updateState],
  );
  const setMatchOvers = useCallback(
    (overs) =>
      updateState((s) => {
        s.matchOvers = parseInt(overs);
      }),
    [updateState],
  );
  ...
  const updateTeamConfig = useCallback(
    (teamId, config) =>
      updateState((s) => {
        Object.assign(s.teams[teamId], config);
      }),
    [updateState],
  );
  const updateActivePlayers = useCallback(
    (updates) =>
      updateState((s) => {
        Object.assign(s.activePlayers, updates);
      }),
    [updateState],
  );
    ...
    <div className='min-h-screen bg-slate-950'>
      ...
      {boundaryAlert && (
        <div className='fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-bounce-in'>
          <div
            className={`rounded-2xl py-4 px-8 border-4 ${boundaryAlert.runs === 6 ? "border-amber-500 bg-amber-950 text-amber-400" : "border-emerald-500 bg-emerald-950 text-emerald-400"}`}
          >
            <div className='teko-font text-6xl'>
              {boundaryAlert.runs === 6 ? "SIX!" : "FOUR!"}
            </div>
      ...
      {wicketAlert && (
        <div className='fixed top-20 left-1/2 -translate-x-1/2 z-50 animate-bounce-in'>
          <div className='rounded-2xl py-4 px-8 border-4 border-red-500 bg-red-950 text-red-400'>
            <div className='teko-font text-6xl'>WICKET!</div>
            <div className='sports-font text-sm'>{wicketAlert.subtitle}</div>
          </div>
        </div>
      )}
      ...
      <header className='sticky top-0 z-40 bg-slate-900/80 backdrop-blur-md border-b border-slate-800 px-6 py-3'>
        <div className='flex items-center justify-between flex-wrap gap-3'>
          ...
          <button
            ...
            className='bg-cyan-600 hover:bg-cyan-500 px-4 py-2 rounded-lg text-sm font-bold transition-all'
          >
        ...
        <div className='flex space-x-2 mt-3'>
          {["scoring", "squad", "graphics"].map((tab) => (
            <button
              ...
              className={`px-5 py-1.5 rounded-lg text-sm font-bold transition-all ${activeTab === tab ? "bg-cyan-600 text-white" : "text-slate-400 hover:text-white hover:bg-slate-800"}`}
            >
      ...
      <main className='p-6'>
          ...
          <div className='grid grid-cols-1 xl:grid-cols-12 gap-6'>
            <div className='xl:col-span-8 space-y-6'>
              ...
              <div className='glass-panel p-6 text-center'>
                <div className='text-7xl text-amber-700 teko-font flex justify-center items-center'>
                  {state.score.runs}/
                  <p className=' text-orange-300'>{state.score.wickets}</p>
                </div>
                <div className='text-lg sports-font text-cyan-400 mt-1'>
                  {formatOvers(state.score.overs, state.score.balls)} /{" "}
                  {state.matchOvers}.0 Overs
                </div>
              ...
              <div className='glass-panel p-5'>
                <div className='flex flex-wrap gap-2 justify-between mb-4'>
                  <div className='flex gap-2'>
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-lg text-sm font-bold ${state.innings === 1 ? "bg-cyan-600" : "bg-slate-700"}`}
                    >
                      1st Inn
                    </button>
                    <button
                      onClick={() => setInnings(2)}
                      className={`px-4 py-1.5 rounded-lg text-sm font-bold ${state.innings === 2 ? "bg-rose-600" : "bg-slate-700"}`}
                    >
                      2nd Inn
                    </button>
                  </div>
                  <div className='flex gap-2'>
                    <button
                      ...
                      className={`px-4 py-1.5 rounded-lg text-sm font-bold ${state.battingTeam === "teamA" ? "bg-cyan-600" : "bg-slate-700"}`}
                    >
                      {state.teams.teamA.short} Bat
                    </button>
                    <button
                      onClick={() => setBattingTeam("teamB")}
                      className={`px-4 py-1.5 rounded-lg text-sm font-bold ${state.battingTeam === "teamB" ? "bg-emerald-600" : "bg-slate-700"}`}
                    >
                      {state.teams.teamB.short} Bat
                    </button>
                  </div>
                  <div className='flex gap-2'>
                    <input
                ...
                <div className='grid grid-cols-4 sm:grid-cols-7 gap-2 mb-4'>
                  {[0, 1, 2, 3, 4, 6, 5].map((r) => (
                    <button
                      key={r}
                      onClick={() => addRuns(r, r === 4 || r === 6)}
                      className={`py-3 rounded-lg font-bold transition-all ${r === 4 ? "bg-emerald-600 hover:bg-emerald-500" : r === 6 ? "bg-amber-600 hover:bg-amber-500" : "bg-slate-700 hover:bg-slate-600"}`}
                    >
                      {r}
                    </button>
                  ))}
                ...
                <div className='grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4'>
                  {["wd", "nb", "by", "lb"].map((type) => (
                    <button
                      ...
                      className='bg-yellow-600/80 hover:bg-yellow-500 py-2 rounded-lg font-bold text-sm uppercase'
                    >
                      {type}
                    </button>
                  ))}
                ...
                <div className='flex gap-3'>
                  ...
                  >
                    <i className='fa-solid fa-skull mr-2'></i> OUT!
                  </button>
                  <button
              ...
              <div className='glass-panel p-5'>
                <div className='grid md:grid-cols-3 gap-4'>
                  <div
                    className={`bg-slate-900/60 rounded-xl p-4 border-2 ${striker?.isStriking ? "border-amber-500" : "border-slate-700"}`}
                  >
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateActivePlayers({
                          bat1Idx: parseInt(e.target.value),
                        })
                      }
                      className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm mb-2'
                    >
                      {battingSquad
                        .filter((p) => !p.isOut)
                        .map((p) => (
                    ...
                    <div className='text-xs text-slate-400'>
                      Runs: {striker?.runs || 0} ({striker?.balls || 0}) |
                      4s/6s: {striker?.fours || 0}/{striker?.sixes || 0}
                    </div>
                  </div>
                  <div
                    className={`bg-slate-900/60 rounded-xl p-4 border-2 ${nonStriker?.isStriking ? "border-amber-500" : "border-slate-700"}`}
                  >
                    <div className='flex justify-between mb-2'>
                      <span className='font-bold'>Non-Striker</span>
                      <input
                        type='radio'
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateActivePlayers({
                          bat2Idx: parseInt(e.target.value),
                        })
                      }
                      className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm mb-2'
                    >
                    ...
                    <div className='text-xs text-slate-400'>
                      Runs: {nonStriker?.runs || 0} ({nonStriker?.balls || 0})
                    </div>
                  </div>
                  <div className='bg-slate-900/60 rounded-xl p-4 border border-slate-700'>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateActivePlayers({
                          bowlerIdx: parseInt(e.target.value),
                        })
                      }
                      className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm mb-2'
                    >
                      {bowlingSquad.map((p) => (
                        <option key={p.id} value={p.id}>
                          {p.name} ({p.wickets}/{p.runsConceded})
                        </option>
                      ))}
                    </select>
                    <div className='text-xs text-slate-400'>
                      Overs: {formatBowlerOvers(bowler?.ballsBowled || 0)} |
                      Econ:{" "}
                      {calculateEconomy(
                ...
                <div className='mt-3 pt-3 border-t border-slate-800 flex justify-between text-sm'>
                  ...
                  <span className='text-amber-400 font-bold'>
                    {state.currentOver.runs}/{state.currentOver.balls}
                  </span>
                  <div className='flex space-x-1'>
                    {state.recentBalls.slice(-6).map((b, i) => (
                      <span
                        key={i}
                        className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${b === "4" ? "bg-emerald-600" : b === "6" ? "bg-amber-500 text-black" : b === "W" ? "bg-red-600" : "bg-slate-700"}`}
                      >
                        {b}
                      </span>
                    ))}
            ...
            <div className='xl:col-span-4 space-y-6'>
              ...
              <div className='glass-panel p-5'>
                ...
                <input
                  ...
                  onChange={(e) =>
                    updateTeamConfig("teamA", {
                      short: e.target.value.toUpperCase(),
                    })
                  }
                  className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm mb-2'
                  placeholder='Team A Short'
                />
                <input
                  value={state.teams.teamA.full}
                  onChange={(e) =>
                    updateTeamConfig("teamA", { full: e.target.value })
                  }
                  className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm mb-2'
                  placeholder='Team A Full'
                />
                <input
                  value={state.teams.teamB.short}
                  onChange={(e) =>
                    updateTeamConfig("teamB", {
                      short: e.target.value.toUpperCase(),
                    })
                  }
                  className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm mb-2'
                  placeholder='Team B Short'
                />
                <input
                  value={state.teams.teamB.full}
                  onChange={(e) =>
                    updateTeamConfig("teamB", { full: e.target.value })
                  }
                  className='w-full bg-slate-800 rounded-lg px-3 py-2 text-sm'
                  placeholder='Team B Full'
                />
                <div className='grid grid-cols-2 gap-2 mt-3'>
                  <input
                    ...
                    onChange={(e) =>
                      updateTeamConfig("teamA", { color1: e.target.value })
                    }
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateTeamConfig("teamA", { color2: e.target.value })
                    }
                    className='h-8 rounded'
                  />
                  <input
                    ...
                    onChange={(e) =>
                      updateTeamConfig("teamB", { color1: e.target.value })
                    }
                    className='h-8 rounded'
                  />
                  <input
                    ...
                    onChange={(e) =>
                      updateTeamConfig("teamB", { color2: e.target.value })
                    }
                    className='h-8 rounded'
                  />
                </div>
        ...
        {activeTab === "graphics" && (
          <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
            ...
            <div className='glass-panel p-5'>
              <h3 className='font-bold mb-3'>OBS Integration</h3>
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      `${window.location.origin}${window.location.pathname}#overlay`,
                    );
                    alert("Copied!");
                  }}
      ...
      {showWicketModal && (
        <div className='fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4'>
          <div className='glass-panel w-full max-w-md p-6 rounded-2xl'>
            ...
            <select
              id='wicketPlayer'
              className='w-full bg-slate-800 rounded-lg px-3 py-2 mb-3'
            >
              {battingSquad
                .filter((p) => !p.isOut && p.hasBatted)
                .map((p) => (
                  <option key={p.id} value={p.id}>
            ...
            <select
              id='wicketIncoming'
              className='w-full bg-slate-800 rounded-lg px-3 py-2 mb-4'
            >
              {battingSquad
              ...
              <button
                onClick={() => {
                  ...
                  const incomingId = parseInt(
                    document.getElementById("wicketIncoming").value,
                  );
...
function BroadcastOverlay() {
  ...
  const needRuns = state.target
    ? Math.max(0, state.target - state.score.runs)
    : 0;
    ...
    <div className='fixed inset-0 w-[1920px] h-[1080px] bg-transparent overflow-hidden pointer-events-none select-none'>
      {boundary && (
        <div className='absolute left-1/2 -translate-x-1/2 bottom-[180px] w-[500px] text-center z-50 animate-bounce-in'>
          <div
            className={`rounded-2xl py-6 px-10 border-4 ${boundary.runs === 6 ? "border-amber-500 bg-amber-950 text-amber-400" : "border-emerald-500 bg-emerald-950 text-emerald-400"}`}
          >
            <div className='teko-font text-8xl'>
      ...
      {state.overlays.lowerThird && (
        <div className='fixed bottom-8 left-[120px] right-[120px] z-50'>
          <div className='flex justify-between items-end mb-1'>
            ...
            <div className='glass-panel px-4 py-1 rounded-t-xl text-xs sports-font'>
              CRR: {crr}{" "}
              {state.target && state.innings === 2 && `| RRR: ${rrr}`}
            </div>
          </div>
          <div
            ...
            <div className='flex items-center space-x-8 px-8'>
              <div className='flex items-center space-x-3'>
                ...
                <span className='text-2xl font-bold text-amber-400'>
                  {striker?.runs}
                  <span className='text-sm text-white'>({striker?.balls})</span>
                </span>
              </div>
              ...
              <div className='flex items-center space-x-3 opacity-70'>
                <div>
                  ...
                  <div className='text-[10px]'>
                    SR:{" "}
                    {calculateStrikeRate(nonStriker?.runs, nonStriker?.balls)}
                  </div>
                </div>
                <span className='text-2xl font-bold'>
                  {nonStriker?.runs}
                  <span className='text-sm'>({nonStriker?.balls})</span>
                </span>
              </div>
            </div>
            <div className='flex items-center bg-black/40 h-full px-6 space-x-4'>
              <div className='text-right'>
                ...
                <div className='text-xs'>
                  {formatBowlerOvers(bowler?.ballsBowled)} - {bowler?.maidens} -{" "}
              ...
              <div className='flex space-x-1'>
                {state.recentBalls.slice(-6).map((ball, i) => (
                  <span
                    key={i}
                    className={`w-7 h-7 flex items-center justify-center text-xs font-bold rounded-full ${ball === "4" ? "bg-emerald-600" : ball === "6" ? "bg-amber-500 text-black" : ball === "W" ? "bg-red-600 animate-pulse" : "bg-slate-800"}`}
                  >
                    {ball}
                  </span>
                ))}
      ...
      {state.overlays.fullFrameVs && (
        <div className='absolute inset-0 bg-slate-950/95 flex flex-col justify-between p-24'>
          <div className='flex justify-between'>
            <span className='teko-font text-5xl text-cyan-400'>
              {state.leagueName}
            </span>
            <span className='sports-font text-2xl bg-cyan-950/80 px-6 py-2 rounded-full'>
              MATCH LIVE
            </span>
          </div>
          <div className='flex items-center justify-center space-x-20 my-auto'>
            <div className='text-center'>
              <div className='teko-font text-9xl'>
                {state.teams.teamA.short}
              </div>
              <div className='sports-font text-2xl text-slate-400'>
                {state.teams.teamA.full}
              </div>
            </div>
            <div className='w-28 h-28 rounded-full bg-cyan-500 flex items-center justify-center teko-font text-6xl text-white'>
              VS
            </div>
            <div className='text-center'>
              <div className='teko-font text-9xl'>
                {state.teams.teamB.short}
              </div>
              <div className='sports-font text-2xl text-slate-400'>
                {state.teams.teamB.full}
              </div>
            </div>
          </div>
          <div className='text-center'>
            <div className='text-slate-500 sports-font text-lg mb-2'>Venue</div>
            <div className='text-3xl font-bold'>{state.venue}</div>
          </div>
        </div>
      )}
    </div>
